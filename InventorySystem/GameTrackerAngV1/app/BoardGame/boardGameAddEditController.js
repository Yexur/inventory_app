        ...
        function ($scope, $modalInstance, dataService, cacheService, row) {
            ...
            if (!!row) {
                ...
                dataService.getItemStatuses().then(
                       function (results) {
                           ...
                           for (var i = 0; i < results.data.length; i++) {
                               ...
                               if (row[0].itemStatus == results.data[i].itemStatus) {
                                   $scope.gameAddEdit.itemStatus = $scope.itemStatuses[i];
                               }
                           }
                       },
                       function (results) {
                ...
                dataService.getItemStatuses().then(
                       function (results) {
                           ...
                           $scope.gameAddEdit.itemStatus = $scope.itemStatuses[0];
                       },
                       function (results) {
                           // on error
                           $scope.hasFormError = true;
            ...
            $scope.submitForm = function () {
                ...
                $scope.game = {
                    ...
                    ItemStatus: $scope.gameAddEdit.itemStatus.label,
                    HobbyId: $scope.gameAddEdit.hobbyId,
                    NewGame: $scope.newGame
                };

                dataService.saveItem($scope.game).then(
                    function (results) {
